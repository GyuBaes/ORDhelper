...
seo때문에 nextjs를 사용했고 개발자도구로 봤을때 head태그 메타태그도 잘 생겨있는데 aws s3에 cloudfront로 배포해서 중간과정에 작업이 더 필요한지 구글에 검색해도 서치에 걸리지 않는다
...
겨우 spa을 만드는데 꽤 많은 시간을 쏟았다 조금이라도 더 나은 방향으로 수정할 부분이 자잘하게 나왔다
무슨 과제최종본_최종최종_최종진짜최종본 이런느낌
실제 서비스되는 일반 사이트는 얼마나 노력이 들어갈까 실감이 났고
같은 클론코딩이지만 뇌빼고 강의보면서 강의 클론코딩 하는거랑 기존사이트에 개선할 방향을 생각해두고 하는 클론코딩은 확실히 다르다 생각했다
최종본을 봐도 자잘하게 기능부분이든 UI부분이든 아쉽지만 노력했다 생각한다 다음 프로젝트를 생각해서 마무리 해야 했지만 피드백의 힘이 대단해서인지 요청사항이나 버그제보가 들어오는 족족 다시 개발해보거나 수정해보기도 했다

usetransition을 통해 사용성을 개선 해보려했지만 오히려 결과물이 더 처참해서 사용하지 못했다.
결국 200개가 넘는 유닛 컴포넌트에 unitlist setunitlist를 넘기다보니 렌더링이 마를날이 없다 프롭 값이 똑같다면 렌더링을 하지않는 react.memo도 별로 힘을 내지 못했다
